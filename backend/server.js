...
const JustTest = require('./models/JustTest')// Import the SavedJob model
...
app.post('/api/signup', async (req, res) => {
  ...
  try {
    ...
    const newUsername = new User({ username, password: hashedPassword });
...
app.post('/api/just-test', async (req, res) => {
  const { mood , sleep } = req.body;

  try {
    const newJustTest = new JustTest({ mood, sleep });
    await newJustTest.save();                                                        
    res.status(201).json({ message: 'User created successfully' });
  } catch (error) {
    console.error('Error during signup:', error);
    res.status(500).json({ error: 'Internal Server Error' });
